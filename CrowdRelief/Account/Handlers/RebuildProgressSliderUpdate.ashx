...
public class RebuildProgressSliderUpdate : IHttpHandler, IReadOnlySessionState
{
	public void ProcessRequest (HttpContext context)
	{
		...
		try
		{
			...
			userSliderTick.UserId = new Guid(survivorId);
			...
			try
			{
				...
				var rebuildUser = (from r in dc.Rebuilds
								   ...
								   where r.SurvivorId == new Guid(survivorId) && or.IsPrimaryOrganization == true
								   select new { r.RebuildId, p.Firstname, p.Lastname, OrganizationName = o.Name }).Take(1).SingleOrDefault();
				...
				if(rebuildUser != null)
				{
					UpdatePost(rebuildUser.Firstname + "'s " + slider.Type + " progress just updated for their home which is being rebuilt by " + rebuildUser.OrganizationName, new Guid(userId), rebuildUser.RebuildId);
				}
			}
			catch(Exception ex)
			{}
