...
{ 
	public partial class ClientLocationMap : BaseWebForm
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			this.Master.ShowSmallFooter = true;
		...
		protected void LoadClientLocationMap()
		{
			...
			var clientLocations = from c in dc.BasicNeedsSurveys
								  ...
								  select new { c.SurveyId, c.SurveyNumber, c.FirstName, c.LastName, c.HomeLossAddress, c.HomeLossCity, c.HomeLossState, c.HomeLossZipCode, c.Latitude, c.Longitude };

			int locationCount = clientLocations.Count();
			...
			foreach(var location in clientLocations)
			{
				...
				string currentStatus = "<h4>Will you volunteer to help?</h4><p>A citizen volunteer is needed to assist this person with recovery.</br>A photographer is needed to take the clients photos.<br>A writer is needed to document clients experience.</p><br>Volunteer by going here <a href=\"/Register.aspx?interactionType=Volunteer\">Volunteer Now</a> or email us at <a href=\"mailto:support@crowdrelief.net\">support@crowdrelief.net</a> to volunteer, reference the survey number below in your email.";
				...
				string needsText = "<h3>Here are the things " + location.FirstName + " needs...</h3>";
				...
				if (!String.IsNullOrEmpty(latitude) && !String.IsNullOrEmpty(longitude))
				{
					...
					if (surveyTypeResult != null)
					{
						...
						if (campaign != null)
						{
							donateToPerson = "<p><a href=\"\\Campaign.aspx?campaignId=" + campaign.BasicNeedsCampaignId + "\" target=\"_blank\">Donate to this person!</a></p>";
							surveyStatus = "Donate";
						}
						...
						switch (surveyStatus)
						{
							...
							case "Donate":
								{
									surveyType = "donate";
									currentStatus = "Donate to help this person replace their homes contents. Vetted by case worker <strong>" + caseManagerName + "</strong>";
								}
								break;
					...
					string needs = needsText + neededItemCount + neededItemsList + currentStatus + donateToPerson;
					string infoWindowContents = "<div>" + needs + "</br><small>Survey number " + location.SurveyNumber + "</small></div>";
