...
{
	public partial class Administration_Surveys : System.Web.UI.Page
	{
			...
			if (!string.IsNullOrEmpty(term))
			{
				...
				if(searchTerms.Count() > 1)
				{
					var surveys = (from s in dc.BasicNeedsSurveys
								   ...
								   select s).ToList().Where(x => searchTerms.Any(y => x.FirstName.Contains(y)) || searchTerms.Any(y => x.LastName.Contains(y)) || searchTerms.Any(y => x.FEMANumber.Contains(y)));
				...
				else
				{
					var surveys = (from s in dc.BasicNeedsSurveys
								   where s.Hidden == false
								   orderby s.CreatedOn descending
								   select s).Where(x => x.FirstName.Contains(term) || x.LastName.Contains(term) || x.FEMANumber.Contains(term));
